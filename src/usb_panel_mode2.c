...
int usb_panel_mode2_send(void)
{
  ...
  while (1) {
    ...
    if (usb_tx_packet_count(PANEL_MODE2_ENDPOINT) < TX_PACKET_LIMIT) {
  ...
  memcpy(tx_packet->buf, usb_panel_mode2_data, PANEL_MODE2_SIZE);
  tx_packet->len = PANEL_MODE2_SIZE;
  usb_tx(PANEL_MODE2_ENDPOINT, tx_packet);
  //serial_print("ok\n");
  return 0;
}
